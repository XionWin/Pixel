...
namespace GBM
{
    ...
    unsafe public struct BufferObject
    {
        ...
        public uint GetFb(Device device)
        {
            if(this.UserData is var fb && fb == IntPtr.Zero)
            {
                ...
                var panelCount = this.PanelCount;
                ...
                {
                    strides[i] = this.PanelStride(i);
                    handles[i] = this.PanelHandle(i);
                    offsets[i] = this.PanelOffset(i);
                }
                ...
                fb = (nint)DRM.Native.GetFB2(device.DeviceGetFD(), width, height, (uint)format, handles, strides, offsets, 0);
                this.SetUserData((nint)fb, new GBM.DestroyUserDataCallback(destroyUserDataCallbackFunc));
            }
            return (uint)fb;
        }
