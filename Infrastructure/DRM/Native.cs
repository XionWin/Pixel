...
namespace DRM
{
    ...
    unsafe public class Native
    {
			...
			uint[] pitches, uint[] offsets, uint flags)
        {
            ...
            uint bufId = 0;
            return drmModeAddFB2(fd, width, height, pixel_format, bo_handles, pitches, offsets, &bufId, flags) is var result && result == 0 ?
            bufId : throw new NotSupportedException("[DRM] drmModeAddFB2 failed.");
        }
        ...
        public static int PageFlip(int fd, uint crtc_id, uint fb_id, PageFlipFlags flags, ref int user_data) =>
